...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="sanos03/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
             steps {
    deleteDir() // 워크스페이스 깨끗하게(이전 내용 잔존 방지)
    git branch: 'main', url: 'https://github.com/sanos03332-rgb/sanos03guestbook2.git'
  }
        }
        stage('Build') {
            steps {
        ...
        stage('Docker Image Push') {
            steps {
                script {
                    docker.withRegistry('', 'sanos03DockerHub_Credential') {
                        oDockImage.push()
                    }
                }
            }
        ...
        stage('Staging Deploy') {
            steps {
                sshagent(credentials: ['sanos03Staging-PrivateKey']) {
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: 'sanos03 제목', to: 'sanos03332@gmail.com')
        ...
        success { 
            slackSend(tokenCredentialId: 'slack-token'
                , channel: '#교육'
                , color: 'good'
                , message: "${JOB_NAME} (${BUILD_NUMBER}) sanos03 빌드가 성공적으로 끝났습니다. Details: (<${BUILD_URL} | here >)")
        }
        failure { 
            slackSend(tokenCredentialId: 'slack-token'
                , channel: '#교육'
                , color: 'danger'
                , message: "${JOB_NAME} (${BUILD_NUMBER}) sanos03 빌드가 실패하였습니다. Details: (<${BUILD_URL} | here >)")
    }
  }
}
